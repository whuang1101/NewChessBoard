...
def no_moving_into_mate(player_number, board, moves_possible, previous_move):
    movement = copy.deepcopy(moves_possible)
    new_board = copy.deepcopy(board)
    if player_number == 1:
        for move in movement:
            new_board = copy.deepcopy(board)
            new_board.board[previous_click[0][0]][previous_click[0][1]].piece.make_move(new_board.board, move[0],
                                                                                        move[1], previous_move)
            if new_board.get_king_position("white") in new_board.all_possible_moves(2):
                moves_possible.remove(move)
            del new_board
    else:
        i = 0
        for move in movement:
            ...
            new_board.board[previous_click[0][0]][previous_click[0][1]].piece.make_move(new_board.board, move[0],
                                                                                        move[1], previous_move)
            if new_board.get_king_position("black") in new_board.all_possible_moves(1):
                moves_possible.remove(move)
            del new_board
            i += 1
    del movement
...
if __name__ == "__main__":
    ...
    while chess_open:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if chess_board.board[white_king_row][white_king_col].piece.in_check or \
                        chess_board.board[black_king_row][black_king_col].piece.in_check:
                    if len(moves) != 0:
                        for i, j in moves:
                            if i == row and j == col:
                    ...
                    no_moving_into_mate(player % 2, chess_board, moves, previous_click)
        ...
        if len(moves) != 0:
            for col, row in moves:
                ...
                elif chess_board.board[col][row].piece.name[0] == "b" or chess_board.board[col][row].piece.name[
                    0] == "w":
    ...
    #         print()
    if white_check_mate:
        print("White Checkmated Black")
    elif black_check_mate:
        print("Black Checkmated White")
